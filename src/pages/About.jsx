  ...
  return (
    <Box>
      ...
      <Container maxWidth="lg" sx={{ py: { xs: 6, md: 12 }, px: { xs: 2, sm: 3 } }}>
        <Typography variant="h3" sx={{ fontWeight: 700, mb: 3, fontSize: { xs: '1.25rem', md: '2.25rem' } }}>Who We Are</Typography>
        <Typography variant="body1" sx={{ color: '#6C757D', lineHeight: 1.8, mb: 4, fontSize: { xs: '0.875rem', md: '1rem' } }}>
        ...
        <Typography variant="body1" sx={{ color: '#6C757D', lineHeight: 1.8, mb: 6, fontSize: { xs: '0.875rem', md: '1rem' } }}>
        ...
        <Typography variant="h4" sx={{ fontWeight: 700, mb: 3, fontSize: { xs: '1.125rem', md: '1.75rem' } }}>Our Core Strengths</Typography>
        <Stack spacing={2} sx={{ mb: 6 }}>
          {strengths.map((strength, idx) => (
            <Box key={idx} sx={{ display: 'flex', gap: 2, alignItems: 'flex-start' }}>
              <CheckCircle size={24} style={{ color: '#DC143C', flexShrink: 0, marginTop: '2px' }} />
              <Typography variant="body1" sx={{ color: '#6C757D', lineHeight: 1.8, fontSize: { xs: '0.875rem', md: '1rem' } }}>{strength}</Typography>
            </Box>
          ))}
        </Stack>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 6, mb: 6 }}>
          <Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
              ...
              <Typography variant="h4" sx={{ fontWeight: 700, fontSize: { xs: '1.125rem', md: '1.75rem' } }}>Mission</Typography>
            </Box>
            <Typography variant="body1" sx={{ color: '#6C757D', lineHeight: 1.8, fontSize: { xs: '0.875rem', md: '1rem' } }}>
          ...
          <Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
              ...
              <Typography variant="h4" sx={{ fontWeight: 700, fontSize: { xs: '1.125rem', md: '1.75rem' } }}>Vision</Typography>
            </Box>
        ...
        <Typography variant="h4" sx={{ fontWeight: 700, mb: 3, fontSize: { xs: '1.125rem', md: '1.75rem' } }}>Values We Stand For</Typography>
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(2, 1fr)' }, gap: 3, mb: 6 }}>
          {values.map((value, idx) => (
            <Box key={idx} sx={{ p: 3, border: '1px solid #E5E7EB', borderRadius: '12px', '&:hover': { borderColor: '#DC143C', boxShadow: '0 4px 12px rgba(220,20,60,0.1)' }, transition: 'all 0.3s' }}>
              <Typography variant="h6" sx={{ fontWeight: 600, mb: 1, color: '#DC143C', fontSize: { xs: '1rem', md: '1.25rem' } }}>{value.title}</Typography>
              <Typography variant="body2" sx={{ color: '#6C757D', fontSize: { xs: '0.8rem', md: '0.875rem' } }}>{value.desc}</Typography>
            </Box>
          ))}
        </Box>

        <Box sx={{ p: 4, background: 'linear-gradient(135deg, #000000 0%, #1a1a1a 100%)', borderRadius: '16px', textAlign: 'center' }}>
          <Typography variant="h5" sx={{ fontWeight: 700, color: 'white', mb: 2, fontSize: { xs: '1.125rem', md: '1.5rem' } }}>Our Promise</Typography>
          <Typography variant="body1" sx={{ color: 'rgba(255,255,255,0.9)', lineHeight: 1.8, maxWidth: '800px', mx: 'auto', fontSize: { xs: '0.875rem', md: '1rem' } }}>
